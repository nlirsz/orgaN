// Arquivo: src/renderer.js (FRONTEND DO ELECTRON - AJUSTADO PARA API)

// --- DADOS ---
// As variáveis 'products' e 'history' não serão mais arrays locais preenchidos pelo localStorage.
// Elas serão populadas dinamicamente pelas chamadas à API.
let editingProductId = null; // Usaremos o _id do MongoDB para edição
let finances = JSON.parse(localStorage.getItem("finances_local")) || []; // Finanças ainda local
...
let currentAuthToken = null; // Para ser usado com login no futuro

// --- FUNÇÕES ---

// Helper para tratamento de erro de fetch e parse de JSON
...
// --- Funções de Produtos (Conectadas à API) ---

async function loadAndRenderPendingProducts() {
    console.log("renderer.js: loadAndRenderPendingProducts - Buscando produtos pendentes...");
    if (!productListEl || !totalProductsEl) {
        console.error("renderer.js: loadAndRenderPendingProducts - Elementos da lista ou total não encontrados.");
        return;
    }
    ...
    try {
        ...
        console.log("renderer.js: loadAndRenderPendingProducts - Produtos recebidos:", pendingProducts);
        ...
        if (Array.isArray(pendingProducts) && pendingProducts.length > 0) {
            ...
            console.log("renderer.js: loadAndRenderPendingProducts - Nenhum produto pendente retornado ou array vazio.");
...
async function handleProductSubmit() {
    ...
    if (!url) {
        alert("A URL do produto é obrigatória para a extração de dados pela IA.");
        return;
    }
    ...
    let requestUrl = `${API_BASE_URL}/products`;
    let mode = "adicionar";
    ...
    try {
        ...
        alert(`Produto "${result.product.name}" ${mode === 'adicionar' ? 'adicionado' : 'atualizado'} com sucesso!`);
...
async function handleMarkAsBought(productId) {
    ...
    try {
        ...
        console.log("Produto marcado como comprado.");
        await loadAndRenderPendingProducts();
        await loadAndRenderPurchasedProducts();
    } catch (error) {
...
function toggleProductButtons(isEditing) {
    if (!addProductBtn || !productActionButtonsDiv || !updateProductBtn || !cancelEditBtn) {
         console.warn("renderer.js: toggleProductButtons - Botões de produto não encontrados.");
         return;
    }
    if (isEditing) {
...
// --- Funções de Finanças (Mantendo localStorage como no seu original) ---
// Cole aqui as suas funções de finanças: renderFinances, renderOrUpdateFinanceChart, 
// addOrUpdateFinanceEntry, setupFinanceEdit, clearFinanceForm, deleteFinanceEntry.
// Elas devem usar a variável 'finances' e chamar 'saveDataLocalFinances()'.
...
function renderFinances() {
    ...
    if (!financeListEl || !financeTotalBalanceEl || !financeChartCanvas) {
        console.error("renderer.js: renderFinances - Elementos da UI de finanças não encontrados.");
        return;
    }
    ...
    sortedFinances.forEach((entry) => {
        ...
        `;
        if(financeListEl) financeListEl.appendChild(li);
    });
    if(financeTotalBalanceEl) financeTotalBalanceEl.textContent = accumulatedBalance.toFixed(2);
...
function setupFinanceEdit(index) {
    ...
    if (!entry) { console.error("setupFinanceEdit: Registro não encontrado no índice", index); return; }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (navButtons.length > 0) {
        ...
        clearProductForm(); 
        // Não precisa chamar loadAndRenderPurchasedProducts aqui se a aba padrão for 'products'
        renderFinances(); 
        
        const firstNavButton = document.querySelector('.nav-btn[data-tab="products"]');
        if (firstNavButton) {
            // Apenas chama o click se não estiver já ativo para evitar loop se for o primeiro carregamento
             if (!firstNavButton.classList.contains('active')) {
                 firstNavButton.click(); 
             } else {
                // Se já está ativo e é a aba de produtos, carrega os produtos
                await loadAndRenderPendingProducts();
             }
        } else if (navButtons.length > 0) {
            navButtons[0].click(); // Ativa a primeira aba disponível se 'products' não existir
        }
    } else {
